...
#include <string.h>
    ...
    void dcript(unsigned char chave_K[], unsigned char seq_bits[], unsigned char buffer[], int mode, unsigned char ant[]) {
        int IP1[] = {4, 1, 3, 5, 7, 2, 8, 6};
        ...
        resultado = getResult(resultado, K1, 2);
        resultado = Permutation(resultado, IP1, 8);
        resultado = getXorada(resultado, ant, 8);
        strcpy(buffer, resultado);
    ...
    char *cript(unsigned char chave_K[], unsigned char seq_bits[], unsigned char buffer[], int mode, unsigned char ant[]) {
        int IP1[] = {4, 1, 3, 5, 7, 2, 8, 6};
        // printf("TA ASSIM ó -> %u\n", seq_bits);
        int qtdOP;   // quantidade de operacoes
        char OP_CD;  // operacao Cifrar ou Decifrar
        ...
        getSubKeys(chave_K, K1, K2);
        if (mode == 1) {
            resultado = getXorada(seq_bits, ant, 8);
        } else {
            resultado = seq_bits;
        }
        resultado = getResult(resultado, K1, 1);
        resultado = getResult(resultado, K2, 2);
        resultado = Permutation(resultado, IP1, 8);
